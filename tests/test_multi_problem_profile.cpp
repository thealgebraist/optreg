...
int main() {
    ...
    // 1. TSP
    std::cout << "Profiling TSP...\n";
    for(int i=0; i<32; ++i) { // 512 is slow for single thread generic bnb, reducing to 32 for prototype
        int n = 10;
        TSPProblem p = TSPProblem::random(n, i);
        
        // Exact
        TSPBranchBound bnb;
        auto t1 = std::chrono::high_resolution_clock::now();
        auto res = bnb.solve(p);
        auto t2 = std::chrono::high_resolution_clock::now();
        double ms = std::chrono::duration<double>(t2-t1).count()*1000;
        
        // Heur
        TSPNearestNeighbor heur;
        auto kres = heur.solve(p);
        
        double gap = (kres.metrics.objective_value - res.metrics.objective_value)/res.metrics.objective_value * 100.0;
        log_res("TSP", n, "BnB_vs_NN", ms, res.metrics.objective_value, res.metrics.iterations, gap);
        
        if (i%10==0) std::cout << "TSP " << i << " done.\n";
    }
    
    // 2. Knapsack
    std::cout << "Profiling Knapsack...\n";
    KnapsackBnB_AMX kbnb;
    KnapsackGreedy kgreedy;
    for(int i=0; i<32; ++i) {
        int n = 15;
        KnapsackProblem p;
        p.n_items = n;
        ...
        for(int j=0; j<n; ++j) { p.weights.push_back(d(rng)); p.values.push_back(d(rng)); }
        
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = kbnb.solve(p);
        auto t2 = std::chrono::high_resolution_clock::now();
        
        auto hr = kgreedy.solve(p);
        
        log_res("Knapsack", n, "BnB_AMX", std::chrono::duration<double>(t2-t1).count()*1000, 
                r.metrics.objective_value, r.metrics.iterations, 
                (r.metrics.objective_value - hr.metrics.objective_value));
    }
    
    // 3. Set Cover
    std::cout << "Profiling SetCover...\n";
    SetCoverBnB_AMX scbnb;
    SetCoverGreedy scgreedy;
    for(int i=0; i<32; ++i) {
        int n_sets = 12;
        int n_elems = 20;
        SetCoverProblem p;
        p.n_sets = n_sets;
        p.n_elements = n_elems;
        std::uniform_int_distribution<int> d(0, n_elems-1);
        for(int s=0; s<n_sets; ++s) { // Ensure likely coverage
            int sz = 5;
            std::vector<size_t> set;
            for(int k=0; k<sz; ++k) set.push_back(d(rng));
            p.sets.push_back(set);
        }
        
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = scbnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        
        auto hr = scgreedy.solve(p);
        log_res("SetCover", n_sets, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 
               (hr.metrics.objective_value - r.metrics.objective_value));
    }
    
    // 4. Vertex Cover
    std::cout << "Profiling VertexCover...\n";
    VertexCoverBnB_AMX vcbnb;
    VertexCoverGreedy vcgreedy;
    for(int i=0; i<32; ++i) {
        int n = 12; // Small for exact
        VertexCoverProblem p;
        p.n_vertices = n;
        std::uniform_real_distribution<double> coin(0,1);
        for(int u=0; u<n; ++u) for(int v=u+1; v<n; ++v) 
            if(coin(rng) < 0.3) p.edges.push_back({(size_t)u, (size_t)v});
            
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = vcbnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        
        auto hr = vcgreedy.solve(p);
        log_res("VertexCover", n, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 
               (hr.metrics.objective_value - r.metrics.objective_value)*100.0/r.metrics.objective_value);
    }
    
    // 5. Clique
    std::cout << "Profiling Clique...\n";
    CliqueBnB_AMX clqbnb;
    for(int i=0; i<32; ++i) {
        int n = 15;
        CliqueProblem p = CliqueProblem::random(n, 0.5, i);
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = clqbnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        log_res("Clique", n, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 0.0);
    }
    
    // 6. Coloring
    std::cout << "Profiling Coloring...\n";
    GraphColoringBnB_AMX colbnb;
    for(int i=0; i<32; ++i) {
        int n = 10;
        GraphColoringProblem p = GraphColoringProblem::random(n, 0.4, i);
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = colbnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        log_res("Coloring", n, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 0.0);
    }

    // 7. Hamiltonian
    std::cout << "Profiling Hamiltonian...\n";
    HamiltonianBnB_AMX hambnb;
    for(int i=0; i<32; ++i) {
        int n = 10;
        HamiltonianProblem p = HamiltonianProblem::random(n, 0.6, i); // High density for existence
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = hambnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        log_res("Hamiltonian", n, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 0.0);
    }

    // 8. SubsetSum
    std::cout << "Profiling SubsetSum...\n";
    SubsetSumBnB_AMX subbnb;
    for(int i=0; i<32; ++i) {
        int n = 20;
        SubsetSumProblem p = SubsetSumProblem::random(n, i);
        auto t1 = std::chrono::high_resolution_clock::now();
        auto r = subbnb.solve(p);
        double ms = std::chrono::duration<double>(std::chrono::high_resolution_clock::now()-t1).count()*1000;
        log_res("SubsetSum", n, "BnB_AMX", ms, r.metrics.objective_value, r.metrics.iterations, 0.0);
    }
